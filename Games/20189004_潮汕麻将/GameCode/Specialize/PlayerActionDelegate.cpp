...
void ExtensionPlayerActionDelegate::AddAction(PlayerPos::Type playerPos, int actionType, float time, PlayerActionDelegateCallback callback, void *data, bool replace)
{
	if(replace)
...
void ExtensionPlayerActionDelegate::OnPlayerResponse(PlayerPos::Type playerPos, int type)
{
	RemoveAction(playerPos, type);
}

void ExtensionPlayerActionDelegate::RemoveAction(PlayerPos::Type playerPos, int type)
{
	PlayerActionDelegateInfo::iterator itr = m_DelegateInfo.find(playerPos);
	if(itr != m_DelegateInfo.end())
	{
...
void ExtensionPlayerActionDelegate::AddListener(PlayerPos::Type playerPos, const RefCntObj<PlayerActionDelegateEnabledListener> &listener)
{
	RemoveListener(playerPos);
	m_EnabledListeners.insert(std::make_pair(playerPos,listener));
}

void ExtensionPlayerActionDelegate::RemoveListener(PlayerPos::Type playerPos)
{
	m_EnabledListeners.erase(playerPos);
}

void ExtensionPlayerActionDelegate::OnEnableActionDelegate(PlayerPos::Type playerPos, bool b)
{
